...
SvgPath.from = function (src) {
  ...
  if (src instanceof SvgPath) {
    ...
    // Clone properies
    s.err = src.err;
    s.segments = src.segments.map(function (sgm) { return { command: sgm.command, values: [...sgm.values] } });
  ...
  this.iterate(function (s, index, x, y) {
    ...
    switch (s.command) {
      ...
      case 'v':
        ...
        result = (p[0] === 0) ? { command: 'v', values: [ p[1] ] } : { command: 'l', values: [ p[0], p[1] ] };
        break;

      case 'V':
        p      = m.calc(x, values[0], false);
        result = (p[0] === m.calc(x, y, false)[0]) ? { command: 'V', values: [ p[1] ] } : { command: 'L', values: [ p[0], p[1] ] };
        break;

      case 'h':
        p      = m.calc(values[0], 0, true);
        result = (p[1] === 0) ? { command: 'h', values: [ p[0] ] } : { command: 'l', values: [ p[0], p[1] ] };
        break;

      case 'H':
        p      = m.calc(values[0], y, false);
        result = (p[1] === m.calc(x, y, false)[1]) ? { command: 'H', values: [ p[0] ] } : { command: 'L', values: [ p[0], p[1] ] };
        break;
      ...
      case 'A':
        ...
        p = m.calc(values[5], values[6], s.command === 'a');

        // Empty arcs can be ignored by renderer, but should not be dropped
        // to avoid collisions with `S A S` and so on. Replace with empty line.
        if ((s.command === 'A' && values[5] === x && values[6] === y) ||
            (s.command === 'a' && values[5] === 0 && values[6] === 0)) {
          result = { command: s.command === 'a' ? 'l' : 'L', values: [ p[0], p[1] ] };
          break;
        }
        ...
        if (e.isDegenerate()) {
          ...
          // command name, s[3] and s[4] are not modified
          result = { command: s.command, values: [ e.rx, e.ry, e.ax, values[3], values[4], p[0], p[1] ] };
        }
      ...
      case 'm':
        ...
        result = { command: 'm', values: [ p[0], p[1] ] };
        break;

      default:
        name       = s.command;
        result     = { command: name, values: [] };
...
SvgPath.prototype.round = function (d) {
  ...
  this.segments.forEach(function (s) {
    var isRelative = (s.command.toLowerCase() === s.command);
    var values = s.values;
    switch (s.command) {
      case 'H':
      case 'h':
...
SvgPath.prototype.iterate = function (iterator, keepLazyStack) {
  ...
  segments.forEach(function (s, index) {
    ...
    var isRelative = (s.command === s.command.toLowerCase());
    var values = s.values;
...
SvgPath.prototype.unarc = function () {
  this.iterate(function (s, index, x, y) {
    var new_segments, nextX, nextY, result = [], name = s.command;
    var values = s.values;
    ...
    if (new_segments.length === 0) {
      return [ { command: name === 'a' ? 'l' : 'L', values: [ values[5], values[6] ] } ];
    }

    new_segments.forEach(function (s) {
      result.push({ command: 'C', values: [ s[2], s[3], s[4], s[5], s[6], s[7] ] });
    });
...
SvgPath.prototype.unshort = function () {
  ...
  this.iterate(function (s, idx, x, y) {
    var name = s.command, nameUC = name.toUpperCase(), isRelative;
    var values = s.values;
